...
const removeMember = TryCatch(async (req, res, next) => {
    ...
    emitEvent(
        ...
        ALERT,
        chat.members,
        `${userThatWillBeRemoved.name} aapko group se nikal diya`
    );
...
const renameGroup = TryCatch(async (req, res, next) => {
    ...
    if (!chat.groupChat) return next(new ErrorHandler("chat no found", 404));
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    // here we have to delete all messages as well as attachments or files cloud
...
const getMessages = TryCatch(async (req, res, next) => {
    ...
    const result_PerPage = 20;
    const skip = (page - 1) * result_PerPage;
    ...
    const [messages, totalMessagesCount] = await Promise.all([
        Message.find({ chat: chatId })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(result_PerPage)
            .populate("sender", "name avatar")
            .lean(),
        Message.countDocuments({ chat: chatId }),
    ]);

    const totalPages = Math.ceil(totalMessagesCount / result_PerPage) || 0;
